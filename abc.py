...
def upload_files_to_tests_folder(files: List[FileContent]) -> Dict[str, Any]:
    ...
    ZIP files: content only (base64 encoded zip data) - extracts to static folder name
    ...
    for file in files:
        try:
            ...
            if file.file_extension == '.zip':
                # Static name for extracted folder
                extracted_folder_name = "extractedplaywrightscript"
                extracted_path = os.path.join(gui_testing_folder, extracted_folder_name)
                
                # Generate unique name for the ZIP file
                unique_zip_name = generate_unique_name("uploaded_repo") + '.zip'
                zip_path = os.path.join(gui_testing_folder, unique_zip_name)
                ...
                result["extracted_repo_path"] = extracted_path
                logger.info(f"Extracted ZIP to: {extracted_path}")
                
                # Add the extracted files to saved_files
                for root, _, files_in_zip in os.walk(extracted_path):
...
def list_files_in_tests_folder(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        directory = './gui_testing_agent/extractedplaywrightscript/zipfile/tests'
