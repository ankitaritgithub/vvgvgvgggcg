...
# Define the base directory where reports are generated
BASE_REPORTS_DIR = "/home/xs427-anksin/Desktop/zipfile/GUIagentTask/Gui_agent_script"

# Mount the GUIagentTask directory so reports inside it can be accessed
app.mount("/GUIagentTask", StaticFiles(directory=BASE_REPORTS_DIR), name="playwright-reports")

@app.get("/")
def root():
    return {"message": "Go to /GUIagentTask/{request_id}/playwright-report/index.html to view reports"}
...
async def execute_tests(request: ChatRequest):
    ...
    try:
        ...
        if not error_message:
            if task_type == "run_tests":
                download_url = f"http://127.0.0.1:8000/GUIagentTask/{extracted_folder_path}/playwright-report/index.html"
            else:
                download_url = f"http://127.0.0.1:8000/GUIagentTask/{extracted_folder_path}/testdata/testdata.csv"
