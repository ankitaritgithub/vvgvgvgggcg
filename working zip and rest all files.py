...
# app.mount("/Gui_agent_script/playwright_tests/playwright-report", StaticFiles(directory="playwright-report"), name="static")

app.mount("/playwright_report", StaticFiles(directory="./Gui_agent_script/playwright_tests/playwright-report"), name="playwright_report")
...
def upload_files_to_tests_folder(files: List[FileContent]) -> List[str]:
    """
    Uploads files to the Gui_agent_script folder with special handling for ZIP files and spec.js files.
    Extracts ZIP files and renames the folder to 'playwright_tests'.
    Returns list of saved filenames.
    """
    saved_files = []
    base_tests_folder = 'Gui_agent_script'
    os.makedirs(base_tests_folder, exist_ok=True)
    
    # Import shutil at the function level to avoid reference before assignment
    import shutil
    ...
    for file in files:
        try:
            ...
            if file.file_extension.lower() == '.zip':
                ...
                temp_extraction_path = os.path.join(base_tests_folder, f"temp_extract_{uuid.uuid4().hex[:8]}")
                os.makedirs(temp_extraction_path, exist_ok=True)
                
                # Save the zip file temporarily
                temp_zip_path = os.path.join(temp_extraction_path, "temp_upload.zip")
                ...
                try:
                    ...
                    final_folder_name = "playwright_tests"
                    final_folder_path = os.path.join(base_tests_folder, final_folder_name)
                    
                    # Remove existing folder if it exists
                    if os.path.exists(final_folder_path):
                        shutil.rmtree(final_folder_path)
                    
                    # Rename the extracted folder to our desired name
                    # First find the extracted folder (there should be only one directory)
                    extracted_items = os.listdir(temp_extraction_path)
                    if len(extracted_items) == 1 and os.path.isdir(os.path.join(temp_extraction_path, extracted_items[0])):
                        # If the zip contained a single folder, we'll rename that folder
                        os.rename(
                            os.path.join(temp_extraction_path, extracted_items[0]),
                            final_folder_path
                        )
                    else:
                        # If the zip contained multiple files, we'll move them directly
                        os.rename(temp_extraction_path, final_folder_path)
                    
                    # Clean up if there's any remaining temp directory
                    if os.path.exists(temp_extraction_path):
                        shutil.rmtree(temp_extraction_path)
                    
                    # Find all spec.js files in the extracted directory
                    for root, _, files_in_zip in os.walk(final_folder_path):
                        for f in files_in_zip:
                            if f.endswith('.spec.js'):
                                relative_path = os.path.relpath(os.path.join(root, f), base_tests_folder)
                                saved_files.append(relative_path)
                    
                    logger.info(f"Extracted and renamed ZIP file to {final_folder_path}")
                    saved_files.append(f"ZIP_EXTRACTED:{final_folder_name}")
                except zipfile.BadZipFile:
                    logger.error(f"Invalid ZIP file: {temp_zip_path}")
            ...
            # Handle spec.js file upload
                ...
                # Determine if we should put this in the playwright folder or base tests folder
                target_folder = base_tests_folder
                
                # Check if playwright_tests folder exists
                playwright_folder = os.path.join(base_tests_folder, 'playwright_tests')
                if os.path.exists(playwright_folder):
                    target_folder = playwright_folder
...
def list_files_in_tests_folder(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        directory = '../GUIagentTask/Gui_agent_script/playwright_tests/tests'
...
def run_playwright_tests(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        target_dir = os.path.expanduser(
            "~/Desktop/zipfile/GUIagentTask/Gui_agent_script/playwright_tests"
        )
        ...
        try:
            ...
            # This block was incorrectly placed before, now it's inside the try
...
async def execute_tests(request: ChatRequest):
    ...
    try:
        ...
        if not error_message:
            if task_type == "run_tests":
                download_url = "/playwright_report/index.html"
            else:
                download_url = "/testdata/testdata.csv"
