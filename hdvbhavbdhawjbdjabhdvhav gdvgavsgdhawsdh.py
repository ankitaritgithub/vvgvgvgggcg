...
# Mount the entire base directory as static files
...
async def execute_tests(request: ChatRequest):
    ...
    try:
        ...
        if not error_message:
            # Extract the request_id and extracted_folder_name for constructing URLs
            request_id = request.id
            extracted_folder_name = final_state.get("extracted_folder")
            
            if extracted_folder_name:
                # Create relative paths for static file serving
                relative_path = f"{request_id}/{extracted_folder_name}"
                
                if task_type == "run_tests":
                    download_url = f"/static/{relative_path}/playwright-report/index.html"
                else:  # generate
...
# At the top of your file where other imports are
from fastapi.staticfiles import StaticFiles
import os
...
# Mount the entire Gui_agent_script directory as a static files directory
app.mount("/static", StaticFiles(directory=BASE_DIR), name="static")
...
*****
# In the /run_tests endpoint, update the download_url assignment:
download_url = None
if not error_message:
    # Get the relative path from the BASE_DIR
    if task_type == "run_tests":
        if extracted_folder_path:
            relative_path = os.path.relpath(extracted_folder_path, BASE_DIR)
            download_url = f"/static/{relative_path}/playwright-report/index.html"
    else:
        if extracted_folder_path:
            relative_path = os.path.relpath(extracted_folder_path, BASE_DIR)
            download_url = f"/static/{relative_path}/testdata/testdata.csv"
