...
def upload_files_to_tests_folder(files: List[FileContent], request_id: str) -> List[str]:
    ...
    Handles both .zip and .spec.js files
    For .zip files: extracts, copies to target directory, and runs npm setup
        ...
        try:
            ...
            if file.file_extension.lower() == '.zip':
                ...
                try:
                    ...
                    target_parent = os.path.expanduser("~/Desktop/zipfile")
                    os.makedirs(target_parent, exist_ok=True)
                    
                    target_dir = os.path.join(target_parent, extracted_folder_name)
                    os.makedirs(target_dir, exist_ok=True)
                    logger.info(f"Created/verified target directory: {target_dir}")
                    
                    # Copy extracted files to target directory
                    try:
                        logger.info(f"Copying files from {extracted_folder_path} to {target_dir}")
                        if os.path.exists(extracted_folder_path):
                            ...
                            raise Exception(f"Source directory does not exist: {extracted_folder_path}")
                        
                        # Change to target directory for npm operations
                        os.chdir(target_dir)
                        ...
                        try:
                            ...
                            logger.info("Cleaning up previous node modules...")
                            ...
                            logger.info("npm setup completed successfully")
                        ...
                        raise Exception(f"Failed to copy files to target directory: {str(e)}")
                    # ===== END TARGET DIRECTORY SETUP =====
...
async def download_file(request_id: str, filename: str):
    """
    Endpoint to download files from a specific request directory
    """
    try:
        safe_filename = os.path.basename(filename)
        file_path = os.path.join(AGENT_SCRIPT_DIR, request_id, safe_filename)
